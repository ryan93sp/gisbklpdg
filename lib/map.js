...
function inisialisasi(){
	...
	bengkel_reg.setStyle(function(feature){
		...
		if (kend==1 && merk > 1){
			color = 'red';
		}else if(kend==1 && merk == 1){
			color = 'yellow'
		}else if(kend==2 && merk > 1){
			color = 'green'
		}else if(kend==2 && merk == 1){
			color = 'blue'
		}
...
function btncarijns(){
	if(selectken.value=='--Pilih Jenis Kendaraan--'){
		...
		refresh();
		$.ajax({
	    url: server+'carijenis.php?ken='+selectken.value+'&merk='+selectmerk.value, data: "", dataType: 'json', success: function (rows){
			loaddata(rows);}
	    });
	}
}
